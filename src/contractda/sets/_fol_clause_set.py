...
class FOLClauseSet(ClauseSet):
    ...
    class BoundarySetGenerator():
        pass

    
    def generate_boundary_set_generator(self, max_count:int = None):
        new_expr = copy.deepcopy(self.expr)
        symbols = new_expr.root.get_symbols()
        for node, set_flag in self._generate_boundary_set_generator(node=new_expr.root, root=new_expr.root, vars=self.vars):
            clause = FOLClauseSet(vars=self.vars, expr=FOLClause._create_clause_by_node(new_expr.root, symbols=symbols))
            yield clause, set_flag, node 
    
    def generate_boundary_set_linear(self, max_count:int = None) -> list[tuple[list[ClauseSet], list[ClauseSet]]]:
        critical_behavior_examples = []
        result = []
        self._generate_boundary_set_linear(result=result, node=self.expr.root, root=self.expr.root, vars=self.vars)
        symbols = self.expr.root.get_symbols()
        for (in_roots, out_roots) in result:
            example_ins = [FOLClauseSet(vars=self.vars, expr=FOLClause._create_clause_by_node(root, symbols=symbols)) for root in in_roots] 
            example_outs = [FOLClauseSet(vars=self.vars, expr=FOLClause._create_clause_by_node(root, symbols=symbols)) for root in out_roots]
    ...
    @staticmethod
    def _generate_boundary_set_linear(result: list[tuple[list[fol_lan.AST_Node], list[fol_lan.AST_Node]]], 
                                      node: fol_lan.AST_Node, 
                                      root: fol_lan.AST_Node, 
                                      vars = None, 
        ...
        if isinstance(node, fol_lan.PropositionNodeBinOp):
            if node.op == "==":
                ...
                if node.op == "||":
                    ...
                    FOLClauseSet._generate_boundary_set_linear(result=result, node=left, root=root, vars=vars, reverse=reverse)
                    node.children[1] = right
                    node.children[0] = fol_lan.PropositionNodeUniOp(op="!", exp1=left)
                    FOLClauseSet._generate_boundary_set_linear(result=result, node=right, root=root,   vars=vars, reverse=reverse)
                    ...
                    FOLClauseSet._newnode_change_op_in_place(node, "&&")
                    node.children[1] = fol_lan.PropositionNodeUniOp(op="!", exp1=right)
                    FOLClauseSet._generate_boundary_set_linear(result=result, node=left, root=root, vars=vars, reverse=reverse^True)
                    node.children[1] = right
                    FOLClauseSet._generate_boundary_set_linear(result=result, node=right, root=root, vars=vars, reverse=reverse)
                    # explore different combination
                    in_roots.append(copy.deepcopy(root))
            ...
            if node.op == "!":
                ch = node.children[0]
                # flip the 
                FOLClauseSet._generate_boundary_set_linear(result=result, node=ch, root=root, vars=vars, reverse=(reverse ^ True))
                out_roots.append(copy.deepcopy(root))
            ...
            if len(node.children) == 1:
                FOLClauseSet._generate_boundary_set_linear(result=result, node=node.children[0], root=root, vars=vars, reverse=reverse)
                return
        if reverse:
    ...
    @staticmethod
    def _generate_boundary_set_generator(node: fol_lan.AST_Node, 
                                         root: fol_lan.AST_Node, 
                                         exclude_empty: bool = False, 
                                         vars = None, 
                                         reverse:bool = False) -> Generator[tuple[fol_lan.AST_Node, bool], None, None]:
        """
        At the time of return, root is modified.
        Return: node: the node of the operator that are focused, 
                bool: whether the set is satisfying (true) or violating (false)
                
        """
        global_from_internal_true = True ^ reverse
        global_from_internal_false = False ^ reverse
        if isinstance(node, fol_lan.PropositionNodeBinOp):
            if node.op == "==":
                FOLClauseSet._newnode_change_op_in_place(node, "!=")
                yield node, global_from_internal_false
                FOLClauseSet._newnode_change_op_in_place(node, "==")
                yield node, global_from_internal_true
                FOLClauseSet._newnode_change_op_in_place(node, "==")
            elif node.op == "<=":
                FOLClauseSet._newnode_change_op_in_place(node, "<")
                yield node, global_from_internal_true
                ...
                if node.op == "||":
                    ...
                    FOLClauseSet._newnode_change_op_in_place(node, "&&")
                    node.children[1] = fol_lan.PropositionNodeUniOp(op="!", exp1=right)
                    yield from FOLClauseSet._generate_boundary_set_generator(node=left, root=root, vars=vars, reverse=reverse)
                    node.children[1] = right
                    node.children[0] = fol_lan.PropositionNodeUniOp(op="!", exp1=left)
                    yield from FOLClauseSet._generate_boundary_set_generator(node=right, root=root, vars=vars, reverse=reverse)
                    node.children[0] = left
                    # explore different combination
                    yield node, global_from_internal_true
                    node.children[1] = fol_lan.PropositionNodeUniOp(op="!", exp1=right)
                    ...
                    FOLClauseSet._newnode_change_op_in_place(node, "&&")
                    node.children[1] = fol_lan.PropositionNodeUniOp(op="!", exp1=right)
                    yield from FOLClauseSet._generate_boundary_set_generator(node=left, root=root, vars=vars, reverse=reverse^True)
                    node.children[1] = right
                    yield from FOLClauseSet._generate_boundary_set_generator(node=right, root=root, vars=vars, reverse=reverse)
                    # explore different combination
                    yield node, global_from_internal_true
            ...
            if node.op == "!":
                ch = node.children[0]
                # flip the 
                yield from FOLClauseSet._generate_boundary_set_generator(node=ch, root=root, vars=vars, reverse=(reverse ^ True))
                yield node, global_from_internal_false
                node.children[0] = fol_lan.PropositionNodeUniOp(op="!", exp1=ch)
                yield node, global_from_internal_true
                node.children[0] = ch
            ...
            if len(node.children) == 1:
                yield from FOLClauseSet._generate_boundary_set_generator(node=node.children[0], root=root, vars=vars, reverse=reverse)
