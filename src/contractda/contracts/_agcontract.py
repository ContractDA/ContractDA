...
class AGContract(ContractBase):
    ...
    def is_independent_decomposition_of(self, other1: ContractBase, other2: ContractBase) -> bool:
        ...
        if isinstance(self.guarantee, ExplicitSet):
            ...
            return self._is_independent_decomposition_of_infinite_set(other1=other1, other2=other2, dmax = 10)
    ...
    def _is_independent_decomposition_of_infinite_set(self, other1: ContractBase, other2: ContractBase, dmax = 5, dinit = 1) -> bool:
        ...
        while d < dmax:
            ...
            if is_counter_env:
                LOG.debug(f"[INDE {d}] The environment {solver._model} does not possess a good fixed point group within {d} neighbors")
            else:
                LOG.debug(f"[INDE {d}] Every targeted environment has a fixed point group smaller than depth {d}")
                ret = True
                break
            ...
            is_counter_env = solver.check()
            if is_counter_env:
                LOG.debug(f"[INDE {d}] The environment {solver._model} has all failed to loop or unstable fixed points...")
                ret = False
                break
            else:
                LOG.debug(f"[INDE {d}] Not finding negative example... keep searching...")
            # increment
            d = d + 1

        print(encoded_loop_array[0])
        print(encoded_loop_array[1])
        print(encoded_loop_array[2])
        print(encoded_loop_array[3])
        LOG.debug(f"===================================================")
        LOG.debug(f"=            Independent Design Result            =")
        LOG.debug(f"===================================================")
        LOG.debug(f"Result: {ret}")
        if ret == True:
            LOG.debug(f"[INDE] Positive proved within depth {d}")
            return ret
        elif ret == False:
            LOG.debug(f"[INDE] Negative proved within depth {d}")
            return ret
        else:# unkno
